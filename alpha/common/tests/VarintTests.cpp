...
TEST(VarintTests, WriteRead32) {
  ...
  ASSERT_EQ(alpha::varint::bulkVarintSize32(data), pos - buffer.get());

  const char* cpos = buffer.get();
  for (int i = 0; i < kNumElements; ++i) {
    ASSERT_EQ(data[i], alpha::varint::readVarint32(&cpos));
  }
  ...
  cpos = buffer.get();
  alpha::varint::bulkVarintDecode32(kNumElements, cpos, bulk.data());
  for (int i = 0; i < kNumElements; ++i) {
...
TEST(VarintTests, WriteRead64) {
  ...
  ASSERT_EQ(alpha::varint::bulkVarintSize64(data), pos - buffer.get());

  const char* cpos = buffer.get();
  for (int i = 0; i < kNumElements; ++i) {
    ASSERT_EQ(data[i], alpha::varint::readVarint64(&cpos));
  }
  ...
  cpos = buffer.get();
  alpha::varint::bulkVarintDecode64(kNumElements, cpos, bulk.data());
  for (int i = 0; i < kNumElements; ++i) {
    ASSERT_EQ(data[i], bulk[i]);
  }
}
