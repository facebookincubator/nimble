...
#ifndef ALPHA_ENCODING_SELECTION_DEBUG_MAX_ITEMS
#define ALPHA_ENCODING_SELECTION_DEBUG_MAX_ITEMS 50
#endif

#ifdef ALPHA_ENCODING_SELECTION_DEBUG
#define ALPHA_SELECTION_LOG(stream) LOG(INFO) << stream << RESET_COLOR
#else
#define ALPHA_SELECTION_LOG(stream)
#endif

#define COMMA ,
#define UNIQUE_PTR_FACTORY_EXTRA(data_type, class, extra_types, ...)           \
  switch (data_type) {                                                         \
    case ::facebook::alpha::DataType::Uint8: {                                 \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<uint8_t>::physicalType extra_types>>(  \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::Int8: {                                  \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<int8_t>::physicalType extra_types>>(   \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::Uint16: {                                \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<uint16_t>::physicalType extra_types>>( \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::Int16: {                                 \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<int16_t>::physicalType extra_types>>(  \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::Uint32: {                                \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<uint32_t>::physicalType extra_types>>( \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::Int32: {                                 \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<int32_t>::physicalType extra_types>>(  \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::Uint64: {                                \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<uint64_t>::physicalType extra_types>>( \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::Int64: {                                 \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<int64_t>::physicalType extra_types>>(  \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::Float: {                                 \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<float>::physicalType extra_types>>(    \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::Double: {                                \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<double>::physicalType extra_types>>(   \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::Bool: {                                  \
      return std::make_unique<class<                                           \
          ::facebook::alpha::TypeTraits<bool>::physicalType extra_types>>(     \
          __VA_ARGS__);                                                        \
    }                                                                          \
    case ::facebook::alpha::DataType::String: {                                \
      return std::make_unique<class<::facebook::alpha::TypeTraits<             \
          std::string_view>::physicalType extra_types>>(__VA_ARGS__);          \
    }                                                                          \
    default: {                                                                 \
      ALPHA_UNREACHABLE(                                                       \
 ...
 private:
  ...
  bitPackedBytes(uint64_t minValue, uint64_t maxValue, uint32_t count) {
    if constexpr (FixedByteWidth) {
      ...
      // dwio/alpha/encodings/FixedBitWidthEncoding.h for full details.
...
class ManualEncodingSelectionPolicyFactory {
 public:
  ...
  static std::vector<std::pair<alpha::EncodingType, float>> parseReadFactors(
      const std::string& val) {
    std::vector<std::pair<alpha::EncodingType, float>> readFactors;
    std::vector<std::string> parts;
    folly::split(';', folly::trimWhitespace(val), parts);
    readFactors.reserve(parts.size());

    std::vector<alpha::EncodingType> possibleEncodings =
        alpha::ManualEncodingSelectionPolicyFactory::possibleEncodings();
...
class ReplayedCompressionPolicy : public alpha::CompressionPolicy {
 public:
  ...
  alpha::CompressionInformation compression() const override {
    if (compressionType_ == alpha::CompressionType::Uncompressed) {
      return {.compressionType = alpha::CompressionType::Uncompressed};
    }

    if (compressionType_ == alpha::CompressionType::Zstd) {
 ...
 private:
  const alpha::CompressionType compressionType_;
  const CompressionOptions compressionOptions_;
};
