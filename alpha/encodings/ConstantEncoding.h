...
ConstantEncoding<T>::ConstantEncoding(
  ...
  ALPHA_CHECK(pos == data.end(), "Unexpected constant encoding end");
}
...
std::string_view ConstantEncoding<T>::encode(
    ...
    Buffer& buffer) {
  if (values.empty()) {
    ALPHA_INCOMPATIBLE_ENCODING("ConstantEncoding cannot be empty.");
  }

  if (selection.statistics().uniqueCounts().size() != 1) {
    ALPHA_INCOMPATIBLE_ENCODING("ConstantEncoding requires constant data.");
  }
