...
void logEncodingSelection(const std::vector<std::string>& source) {
  ...
  auto policy = std::unique_ptr<alpha::ManualEncodingSelectionPolicy<T>>(
      static_cast<alpha::ManualEncodingSelectionPolicy<T>*>(
          facebook::alpha::ManualEncodingSelectionPolicyFactory{
              FLAGS_read_factors.empty()
                  ? alpha::ManualEncodingSelectionPolicyFactory::
                        defaultReadFactors()
                  : alpha::ManualEncodingSelectionPolicyFactory::
              ...
              .createPolicy(alpha::TypeTraits<T>::dataType)
              .release()));
  ...
  LOG(INFO) << "Encoding: " << GREEN
            << alpha::tools::getEncodingLabel(serialized) << RESET_COLOR;
}
...
#define DATA_TYPE(type) \
  {toString(alpha::TypeTraits<type>::dataType), logEncodingSelection<type>}
  std::unordered_map<
      std::string,
