...
// This method mimics AlignedBuffer::allocate() logic.
template <typename T>
uint64_t getAlignedBufferAllocationSize(uint64_t size) {
  const size_t tSize =
      (sizeof(T) * size / velox::AlignedBuffer::kSizeofAlignedBuffer) *
      velox::AlignedBuffer::kSizeofAlignedBuffer;
  return tSize + velox::AlignedBuffer::kSizeofAlignedBuffer +
      velox::AlignedBuffer::kPaddedSize;
}

// Get the usable buffer capacity based on the size logic in
// AlignedBuffer::allocate().
template <typename T>
uint64_t getAlignedBufferCapacity(uint64_t size) {
  return (getAlignedBufferAllocationSize<T>(size) -
          velox::AlignedBuffer::kPaddedSize) /
      sizeof(T);
}
...
TYPED_TEST(NullStreamDataTest, EnsureAdditionalNullsCapacity) {
  ...
  MockGrowthPolicy growthPolicy;
  {
    ...
    EXPECT_CALL(
        growthPolicy,
        getExtendedCapacity(1030, getAlignedBufferCapacity<bool>(20)))
  ...
  EXPECT_EQ(actualCapacity1, getAlignedBufferCapacity<bool>(20));
  ...
  EXPECT_GE(actualCapacity2, actualCapacity1);
  EXPECT_EQ(actualCapacity2, getAlignedBufferCapacity<bool>(1030));
  EXPECT_EQ(nonNulls.size(), 20);
...
TYPED_TEST(ContentStreamDataTest, EnsureMutableDataCapacity) {
  ...
  MockGrowthPolicy growthPolicy;
  {
    ...
    EXPECT_CALL(
        growthPolicy,
        getExtendedCapacity(1000, getAlignedBufferCapacity<int32_t>(20)))
  ...
  EXPECT_GE(actualCapacity1, 20);
  EXPECT_EQ(actualCapacity1, getAlignedBufferCapacity<int32_t>(20));
  ...
  EXPECT_GE(actualCapacity2, actualCapacity1);
  EXPECT_EQ(actualCapacity2, getAlignedBufferCapacity<int32_t>(1000));
  EXPECT_EQ(data.size(), 20);
