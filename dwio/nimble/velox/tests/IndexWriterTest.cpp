...
class IndexWriterTest : public velox::exec::test::OperatorTestBase {
 protected:
  ...
  void SetUp() override {
    ...
    context_ = std::make_unique<FieldWriterContext>(*pool_);
  ...
  std::unique_ptr<FieldWriterContext> context_;
...
TEST_F(IndexWriterTest, createWithNoConfig) {
  ...
  auto writer = IndexWriter::create(std::nullopt, type, *context_);
...
TEST_F(IndexWriterTest, createWithValidConfig) {
  ...
  for (auto compressionType :
    ...
    auto writer = IndexWriter::create(config, type, *context_);
...
TEST_F(IndexWriterTest, createWithInvalidConfig) {
...

  {
    ...
    VELOX_ASSERT_USER_THROW(
        IndexWriter::create(config, type, *context_), "Field not found");
  }

  {
    ...
    NIMBLE_ASSERT_THROW(
        IndexWriter::create(config, type, *context_),
...
TEST_F(IndexWriterTest, emptyWriteFinishStripe) {
  ...
  auto writer = IndexWriter::create(config, type_, *context_);
  ASSERT_NE(writer, nullptr);

  EXPECT_FALSE(writer->hasKeys());
  auto stream = writer->finishStripe(*encodingBuffer_);
...
TEST_F(IndexWriterTest, enforceKeyOrderInvalidWithinBatch) {
  for (bool enforceKeyOrder : {true, false}) {
    ...
    if (enforceKeyOrder) {
      NIMBLE_ASSERT_THROW(
          writer->write(batch), "Encoded keys must be in non-descending order");
    } else {
      EXPECT_NO_THROW(writer->write(batch));
      writer->encodeStream(*encodingBuffer_);
      auto stream = writer->finishStripe(*encodingBuffer_);
...
TEST_P(IndexWriterDataTest, writeAndFinishStripeNonChunked) {
  ...
  for (const auto& testCase : testCases) {
    ...
    ASSERT_TRUE(stream->chunks[0].firstKey.has_value());
    ASSERT_TRUE(stream->chunks[0].lastKey.has_value());
    ...
    keyEncoder->encode(
        makeKeyInput(testCase.batches.front().front()), encodedKeys, keyBuffer);
    EXPECT_EQ(stream->chunks[0].firstKey.value(), encodedKeys[0]);

    // Verify lastKey matches the last value of the last batch.
    encodedKeys.clear();
    keyEncoder->encode(
        makeKeyInput(testCase.batches.back().back()), encodedKeys, keyBuffer);
    EXPECT_EQ(stream->chunks[0].lastKey.value(), encodedKeys[0]);
  }
}
...
TEST_P(IndexWriterDataTest, writeAndFinishStripeChunked) {
  ...
  for (const auto& testCase : testCases) {
    ...
    while (writer->encodeChunk(false, false, *encodingBuffer_)) {
    }

    while (writer->encodeChunk(false, true, *encodingBuffer_)) {
    }
    ...
    for (size_t i = 0; i < stream->chunks.size(); ++i) {
      ...
      ASSERT_TRUE(chunk.firstKey.has_value());
      ASSERT_TRUE(chunk.lastKey.has_value());

      // Verify firstKey matches the value at chunk start offset.
      EXPECT_EQ(chunk.firstKey.value(), allEncodedKeys[rowOffset]);

      // Verify lastKey matches the value at chunk end offset.
      EXPECT_EQ(
          chunk.lastKey.value(),
          allEncodedKeys[rowOffset + chunk.rowCount - 1]);
...
TEST_P(IndexWriterDataTest, indexChunkSizeControl) {
  ...
  for (const auto& testCase : testCases) {
    ...
    while (writer->encodeChunk(
        testCase.ensureFullChunks, /*lastChunk=*/false, *encodingBuffer_)) {
      ++chunksBeforeLast;
    }
    ...
    while (writer->encodeChunk(
        /*ensureFullChunks=*/false, /*lastChunk=*/true, *encodingBuffer_)) {
    }
    ...
    for (size_t i = 0; i < stream->chunks.size(); ++i) {
      ...
      // Verify firstKey matches the value at chunk start offset.
      ASSERT_EQ(chunk.firstKey.value(), allEncodedKeys[rowOffset]);
