...
TEST(SchemaSerializationTest, ScalarInt8) {
  ...
  SCHEMA(schemaBuilder, ROW({{"field", TINYINT()}}));
...
TEST(SchemaSerializationTest, ScalarTypes) {
  ...
  for (const auto& tc : cases) {
    ...
    SCHEMA(
        schemaBuilder,
        ROW({{"f", builder.createScalarTypeBuilder(tc.expectedKind)}}));
    auto root = roundTrip(schemaBuilder);
    auto& row = root->asRow();
...
TEST(SchemaSerializationTest, RowType) {
  ...
  SCHEMA(
      schemaBuilder,
      ROW({{"a", TINYINT()}, {"b", INTEGER()}, {"c", STRING()}}));
  auto root = roundTrip(schemaBuilder);
  ASSERT_EQ(Kind::Row, root->kind());
  auto& row = root->asRow();
...
TEST(SchemaSerializationTest, NestedComplex) {
  ...
  SchemaBuilder schemaBuilder;
  SCHEMA(schemaBuilder, ROW({{"nested", ARRAY(MAP(STRING(), BIGINT()))}}));
  auto root = roundTrip(schemaBuilder);
  auto& row = root->asRow();
  ASSERT_EQ(1, row.childrenCount());
