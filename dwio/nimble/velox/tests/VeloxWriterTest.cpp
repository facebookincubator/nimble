...
#include<utility>
  ...
  TEST_F(VeloxWriterTest, schemaGrowthExtraSubField) {
    ...
    const auto type =
        velox::ROW({"c0"}, {velox::ROW({"f1"}, {velox::BIGINT()})});
    velox::test::VectorMaker vectorMaker{leafPool_.get()};
    ...
    for (uint32_t stripeIndex = 0; stripeIndex < stripeCount; ++stripeIndex) {
      ...
      for (uint32_t streamId = 0; streamId < streamLoaders.size(); ++streamId) {
        ...
        while (chunkedStream.hasNext()) {
          ...
          const uint32_t rowCount = *reinterpret_cast<const uint32_t*>(
              chunk.data() + kRowCountOffset);
  ...
  TEST_P(StripeRawSizeFlushPolicyTest, StripeRawSizeFlushPolicy) {
    ...
    auto batches = generateBatches(
        type, GetParam().batchCount, 4000, 20221110, *leafPool_);
      ...
      for (auto i = 0; i < tablet->stripeCount(); ++i) {
...

        {
          ...
          auto capture =
              nimble::EncodingLayoutCapture::capture(chunkedStream.nextChunk());
          EXPECT_EQ(
              nimble::EncodingType::MainlyConstant, capture.encodingType());
          EXPECT_EQ(
              nimble::EncodingType::Trivial,
              capture
                  .child(
  ...
  TEST_F(VeloxWriterTest, combineMultipleLayersOfDictionaries) {
    ...
    auto wrapInDictionary = [&](const std::vector<vector_size_t>& indices,
      ...
      auto buf = AlignedBuffer::allocate<vector_size_t>(
          indices.size(), leafPool_.get());
      memcpy(
    ...
    nimble::VeloxReader reader(
        &readFile, *leafPool_, nullptr, std::move(params));
    VectorPtr result;
    ASSERT_TRUE(reader.next(4, result));
    ASSERT_EQ(result->size(), 4);
    auto* c0 =
        result->asChecked<RowVector>()->childAt(0)->asChecked<RowVector>();
...
#define ASSERT_CHUNK_COUNT(count, chunked) \
    ...
    testChunks(
        *rootPool_,
        ...
        [&](const auto& tablet) {
          ...
          auto streamLoaders = tablet.load(
              stripeIdentifier, std::array<uint32_t, 4>{0, 1, 2, 3});
  ...
  TEST_F(VeloxWriterTest, ignoreTopLevelNulls) {
    ...
    // Write ignoring top level nulls with encoded input
    {
      ...
      rowVector = velox::BaseVector::wrapInDictionary(
          nullptr, indices, kSize, rowVector);
      auto expected = std::make_shared<velox::RowVector>(
          leafPool_.get(),
          type,
          nullptr,
  ...
  INSTANTIATE_TEST_CASE_P(
      ...
      ::testing::Values(
          ...
          TimestampTestCase{
              "Negative999ms", velox::Timestamp::fromMillis(-999)},
          TimestampTestCase{
              "Negative1000ms", velox::Timestamp::fromMillis(-1000)},
          TimestampTestCase{
              "Negative1500ms", velox::Timestamp::fromMillis(-1500)},
          ...
          TimestampTestCase{
              "MaxMicros", velox::Timestamp(INT64_MAX / 1'000'000, 999)},
          TimestampTestCase{
              "MinMicros", velox::Timestamp(INT64_MIN / 1'000'000, 0)}));
  ...
  INSTANTIATE_TEST_CASE_P(
      ...
      ::testing::Values(
          StripeRawSizeFlushPolicyTestCase{
              .batchCount = 50, .rawStripeSize = 256 << 10, .stripeCount = 4},
          StripeRawSizeFlushPolicyTestCase{
              .batchCount = 100, .rawStripeSize = 256 << 10, .stripeCount = 7},
          StripeRawSizeFlushPolicyTestCase{
              .batchCount = 100, .rawStripeSize = 256 << 11, .stripeCount = 4},
          StripeRawSizeFlushPolicyTestCase{
              .batchCount = 100, .rawStripeSize = 256 << 12, .stripeCount = 2},
          StripeRawSizeFlushPolicyTestCase{
              .batchCount = 100,
   ...
   protected:
    ...
    void verifyPositionIndex(const nimble::TabletReader& tablet) {
      for (uint32_t stripeIdx = 0; stripeIdx < tablet.stripeCount();
        ...
        for (uint32_t streamId = 0; streamId < streamLoaders.size();
             ++streamId) {
          if (!streamLoaders[streamId]) {
            continue;
          }
          ...
          // Method 1: Verify using sequential InMemoryChunkedStream iteration
          {
            ...
            while (chunkedStream.hasNext()) {
              ...
              EXPECT_EQ(
                  encoding->rowCount(), expectedChunkRowCounts[chunkIndex])
                  << "Stripe " << stripeIdx << " stream " << streamId
                  << " chunk " << chunkIndex
                  << " row count mismatch (sequential)";
              ++chunkIndex;
            }
          ...
          for (uint32_t i = 0; i < numChunksInStripe; ++i) {
            ...
            if (i + 1 < numChunksInStripe) {
              chunkSize = streamStats.chunkOffsets[prevChunkCount + i + 1] -
                  chunkOffset;
            } else {
      ...
      for (const auto& batch : batches) {
        ...
        for (velox::vector_size_t i = 0; i < batch->size(); ++i) {
          ...
          EXPECT_EQ(fileRowId, expectedFileRowId)
              ...
              << fileRowId << " (stripe " << stripeIndex
              << ", stripe start row " << stripeStartRows[stripeIndex]
  ...
  nimble::VeloxReadParams nonLegacyReadParams() {
    ...
    params.encodingFactory =
      ...
      return nimble::EncodingFactory().create(pool, data, std::move(stringBufferFactory));
    };
    return params;
  }
  ...
  TEST_F(VeloxWriterTest, encodingLayoutDeltaMultiStripe) {
    ...
    while (reader.next(kBatchSize, result)) {
      for (int32_t i = 0; i < result->size(); ++i) {
        ASSERT_TRUE(
            result->equalValueAt(batches[batchIdx].get(), i, batchOffset))
