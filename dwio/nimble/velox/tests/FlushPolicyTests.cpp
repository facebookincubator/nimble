...
TEST_F(ChunkFlushPolicyTest, InitialNoMemoryPressure) {
  ChunkFlushPolicy policy(ChunkFlushPolicyConfig{
...
TEST_F(ChunkFlushPolicyTest, MemoryPressureChunkingLifecycle) {
  ...
  EXPECT_EQ(policy.shouldChunk(progress3), ChunkDecision::Chunk);
  ...
  EXPECT_EQ(policy.shouldChunk(progress4), ChunkDecision::None);
}
...
TEST(FlushPolicyTest, StripeRawSizeFlushPolicyTest) {
  ...
  EXPECT_EQ(policy.shouldFlush(progress2), FlushDecision::None);
}
