...
TEST_P(E2EFilterTest, integerRleCustomSeed_4049269257) {
  ...
  // Subfield pruning test validation doesn't work correctly when data is sorted
  // for index testing.
  if (indexEnabled()) {
    GTEST_SKIP() << "Skipping for index-enabled case";
  }
  seed_ = 4049269257;
  testWithTypes(
      "short_val:smallint,"
...
TEST_P(E2EFilterTest, lazyStruct) {
  // Lazy loading verification compares row-by-row with expected batches,
  // which doesn't work correctly when data is sorted for index testing.
  if (indexEnabled()) {
    GTEST_SKIP() << "Skipping for index-enabled case";
  }
  testWithTypes(
...
TEST_P(E2EFilterTest, filterStruct) {
  // Struct filter validation doesn't work correctly when data is sorted for
  // index testing.
  if (indexEnabled()) {
    GTEST_SKIP() << "Skipping for index-enabled case";
  }
...
TEST_P(E2EFilterTest, mutationCornerCases) {
  // This test doesn't use testWithTypes, so skip for index-enabled case.
  if (indexEnabled()) {
    GTEST_SKIP() << "Skipping for index-enabled case";
  }
...
TEST_P(E2EFilterTest, timestampNestedArray) {
  // Nested array timestamp validation doesn't work correctly when data is
  // sorted for index testing.
  if (indexEnabled()) {
    GTEST_SKIP() << "Skipping for index-enabled case";
  }
  testWithTypes(
...
TEST_P(E2EFilterTest, timestampNestedMap) {
  // Nested map timestamp validation doesn't work correctly when data is sorted
  // for index testing.
  if (indexEnabled()) {
    GTEST_SKIP() << "Skipping for index-enabled case";
  }
...
TEST_P(E2EFilterTest, timestampArrayInMap) {
  // Array-in-map timestamp validation doesn't work correctly when data is
  // sorted for index testing.
  if (indexEnabled()) {
    GTEST_SKIP() << "Skipping for index-enabled case";
  }
...
TEST_P(E2EFilterTest, timestampMapInArray) {
  // Map-in-array timestamp validation doesn't work correctly when data is
  // sorted for index testing.
  if (indexEnabled()) {
    GTEST_SKIP() << "Skipping for index-enabled case";
  }
...
TEST_P(E2EFilterTest, timestampStructInArray) {
  // Struct-in-array timestamp validation doesn't work correctly when data is
  // sorted for index testing.
  if (indexEnabled()) {
    GTEST_SKIP() << "Skipping for index-enabled case";
  }
...
TEST_P(E2EFilterTest, timestampArrayInStruct) {
  // Array-in-struct timestamp validation doesn't work correctly when data is
  // sorted for index testing.
  if (indexEnabled()) {
    GTEST_SKIP() << "Skipping for index-enabled case";
  }
