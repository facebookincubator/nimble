...
#include "fb_velox/nimble/reader/NimbleReader.h"
...
DEFINE_bool(use_dwrf, false, "Use the DWRF writer instead of Nimble");
...
class NimbleWriter : public Writer {
 public:
  NimbleWriter(
    ...
    writer_ = std::make_unique<nimble::VeloxWriter>(
 ...
 private:
  std::string buffer_;
  std::shared_ptr<velox::memory::MemoryPool> leafPool_;
  std::unique_ptr<nimble::VeloxWriter> writer_;
}; // NimbleWriter
} // namespace
...
int main(int argc, char* argv[]) {
  ...
  auto readerFactory = facebook::velox::nimble::NimbleReaderFactory();
  ...
  if (FLAGS_use_dwrf) {
    ...
    writer = std::make_unique<NimbleWriter>(*rootPool, leafPool, schema);
  }
