...
struct StripeLocation {
  StripeLocation(size_t _stripeIndex, uint32_t _stripeGroupIndex)
      : stripeIndex(_stripeIndex), stripeGroupIndex(_stripeGroupIndex) {}

  size_t stripeIndex;
  uint32_t stripeGroupIndex;

  bool operator==(const StripeLocation& other) const {
    return std::tie(stripeIndex, stripeGroupIndex) ==
        std::tie(other.stripeIndex, other.stripeGroupIndex);
  }
};
...
/// - On-demand access to stripe group indices and index group metadata
...
///   if (location) {
///     auto metadata = index->stripeIndexGroupMetadata(
///         location->stripeGroupIndex);
///   }
class TabletIndex {
 public:
  ...
  /// @return StripeLocation containing stripe index and stripe group index if
  ...
  /// Returns metadata for a specific stripe index group.
  ///
  /// @param stripeIndexGroupId The zero-based stripe index group ID
  /// @return Metadata section containing offset, size, and
  ///         compression info for the stripe index group
  MetadataSection stripeIndexGroupMetadata(uint32_t stripeIndexGroupId) const;
