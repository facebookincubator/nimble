...
#include "dwio/nimble/serializer/BatchedStreamDecoder.h"
...
void addBatches(
  ...
  for (const auto& batch : input.batches) {
    if (batch.hasStream(streamOffset)) {
      decoder.addBatch(startRow, batch.stream(streamOffset), batch.version);
    }
...
TEST_F(BatchedStreamDecoderTest, addBatchRejectsEmptySegment) {
  ...
  NIMBLE_ASSERT_THROW(
      decoder.addBatch(
          0, std::string_view{}, SerializationVersion::kSerialization),
