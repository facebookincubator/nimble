...
SerializationVersion upgradeReadOnlyProjectVersion(
    SerializationVersion version) {
  if (version == SerializationVersion::kLegacyCompact) {
    LOG_FIRST_N(WARNING, 10)
        << "StreamSelector constructed with projectVersion=kLegacyCompact "
...
// kTablet is rejected because the projection does not strip chunk headers.
...
StreamSelector::StreamSelector(
    ...
    : projectVersion_(upgradeReadOnlyProjectVersion(options.projectVersion)),
...
// output chain.
std::vector<uint32_t> selectStreamsContiguousUnsorted(
...
// Fast path for selectStreamsContiguousUnsorted when inputStreamIndices are
...
std::vector<uint32_t> selectStreamsContiguousSorted(
    const folly::IOBuf& input,
    size_t dataOffset,
    const std::vector<uint32_t>& streamIndices,
    const std::vector<uint32_t>& streamSizes,
...
// Fast path for selectStreamsChainedUnsorted when inputStreamIndices are
...
std::vector<uint32_t> selectStreamsChainedSorted(
...
std::vector<uint32_t> selectStreamsChainedUnsorted(
    folly::io::Cursor& cursor,
    const std::vector<uint32_t>& streamIndices,
    const std::vector<uint32_t>& streamSizes,
...
ProjectedStreams StreamSelector::selectStreams(
    const folly::IOBuf& input) const {
  const auto inputVersion = getAndValidateInputVersion(input);

  if (!input.isChained()) {
    return selectContiguous(input, inputVersion);
  }
  return selectChained(input, inputVersion);
}

ProjectedStreams StreamSelector::selectContiguous(
    const folly::IOBuf& input,
  ...
  // Extract selected streams as zero-copy sub-range clones into a header-less
  // buffer.
  auto streamData = folly::IOBuf::create(0);
  ...
  if (inputStreamsSorted_) {
    outputStreamSizes = selectStreamsContiguousSorted(
        input,
        dataOffset,
        streamIndices,
        streamSizes,
    ...
    outputStreamSizes = selectStreamsContiguousUnsorted(
        input,
        dataOffset,
        streamIndices,
        streamSizes,
  ...
  return ProjectedStreams{
      .rowCount = rowCount,
      .version = projectVersion_,
      .requiresNullBarrier = outputRequiresNullBarrier,
      .streamSizes = std::move(outputStreamSizes),
      .streamData = std::move(*streamData)};
}

ProjectedStreams StreamSelector::selectChained(
    const folly::IOBuf& input,
    SerializationVersion inputVersion) const {
  ...
  // Extract selected streams as zero-copy clones via cursor into a header-less
  // buffer.
  auto streamData = folly::IOBuf::create(0);
  bool outputRequiresNullBarrier = false;
  std::vector<uint32_t> outputStreamSizes;
  if (inputStreamsSorted_) {
    outputStreamSizes = selectStreamsChainedSorted(
        cursor,
        streamIndices,
        streamSizes,
        inputStreamIndices_,
    ...
    outputStreamSizes = selectStreamsChainedUnsorted(
        cursor,
        streamIndices,
        streamSizes,
