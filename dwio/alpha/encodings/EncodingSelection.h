...
// Overview:
// Alpha has many encoding to chose from. Each encoding yields different results
// on different shapes of data.
// In addition, Alpha's encodings are nested (e.g. encodings may contain other
// encodings, to encode sub streams of data).
// To be able to successfully encode an Alpha stream, we must build an encoding
// tree, optimized for the given data.
// Alpha is using a top-down (greedy) selection approach, where, by inspecting
// the top level data, we pick an encoding, and use it to encode the data. If
// this is a nested encoding, we then break down the data, and apply the same
// algorithm on each nested encoding.
// Once we pick an encoding for a specific level, we commit to this selection
// and never go back and change it.
//
