...
T randomValue(RNG&& rng, alpha::Buffer& buffer) {
...
template <typename T, typename RNG, bool SmallNumbers = false>
inline alpha::Vector<T>
generateRandomData(RNG&& rng, alpha::Buffer& buffer, int count) {
  alpha::Vector<T> data{&buffer.getMemoryPool()};
  data.reserve(count);
...
inline alpha::Vector<T>
generateConstData(RNG&& rng, alpha::Buffer& buffer, int count) {
  alpha::Vector<T> data{&buffer.getMemoryPool()};
  data.reserve(count);
...
template <typename T, typename RNG>
inline alpha::Vector<T>
generateFixedBitWidthData(RNG&& rng, alpha::Buffer& buffer, int count) {
  alpha::Vector<T> data{&buffer.getMemoryPool()};
  data.reserve(count);
  ...
  LOG(INFO) << alpha::bits::printBits(mask);
  physicalType val =
...
void writeFile(
    ...
    uint32_t rowCount,
    std::function<alpha::Vector<
        typename E::cppDataType>(RNG&&, alpha::Buffer&, int)> dataGenerator =
        [](RNG&& rng, alpha::Buffer& buffer, int count) {
  ...
  auto identifier = fmt::format(
      "{}_{}_{}",
      alpha::test::Encoder<E>::encodingType(),
      toString(alpha::TypeTraits<typename E::cppDataType>::dataType),
      rowCount);
  ...
  std::optional<alpha::Vector<bool>> nulls;

  if (alpha::test::Encoder<E>::encodingType() ==
  ...
  for (auto compressionType :
    ...
    if constexpr (
        alpha::test::Encoder<E>::encodingType() ==
        alpha::EncodingType::Nullable) {
      encoded = alpha::test::Encoder<E>::encodeNullable(
          buffer, data, nulls.value(), compressionType);
    } else {
      encoded = alpha::test::Encoder<E>::encode(buffer, data, compressionType);
    }
    {
      std::ofstream file{
          fmt::format(
...
#define _WRITE_NUMERIC_FILES_(encoding, rowCount, type)                        \
  writeFile##type<alpha::encoding<int8_t>>(rng, FLAGS_output_dir, rowCount);   \
  writeFile##type<alpha::encoding<uint8_t>>(rng, FLAGS_output_dir, rowCount);  \
  writeFile##type<alpha::encoding<int16_t>>(rng, FLAGS_output_dir, rowCount);  \
  writeFile##type<alpha::encoding<uint16_t>>(rng, FLAGS_output_dir, rowCount); \
  writeFile##type<alpha::encoding<int32_t>>(rng, FLAGS_output_dir, rowCount);  \
  writeFile##type<alpha::encoding<uint32_t>>(rng, FLAGS_output_dir, rowCount); \
  writeFile##type<alpha::encoding<int64_t>>(rng, FLAGS_output_dir, rowCount);  \
  writeFile##type<alpha::encoding<uint64_t>>(rng, FLAGS_output_dir, rowCount); \
  writeFile##type<alpha::encoding<float>>(rng, FLAGS_output_dir, rowCount);    \
  writeFile##type<alpha::encoding<double>>(rng, FLAGS_output_dir, rowCount);

#define _WRITE_FILES_(encoding, rowCount, type)                            \
  _WRITE_NUMERIC_FILES_(encoding, rowCount, type)                          \
  writeFile##type<alpha::encoding<bool>>(rng, FLAGS_output_dir, rowCount); \
  writeFile##type<alpha::encoding<std::string_view>>(                      \
...
#define WRITE_NON_BOOL_FILES(encoding, rowCount) \
  _WRITE_NUMERIC_FILES_(encoding, rowCount, )    \
  writeFile<alpha::encoding<std::string_view>>(rng, FLAGS_output_dir, rowCount);
...
int main(int argc, char* argv[]) {
  ...
  for (auto rowCount : {0, 256}) {
    ...
    writeFile<alpha::SparseBoolEncoding>(rng, FLAGS_output_dir, rowCount);
    writeFile<alpha::VarintEncoding<int32_t>>(rng, FLAGS_output_dir, rowCount);
    writeFile<alpha::VarintEncoding<uint32_t>>(rng, FLAGS_output_dir, rowCount);
    writeFile<alpha::VarintEncoding<int64_t>>(rng, FLAGS_output_dir, rowCount);
    writeFile<alpha::VarintEncoding<uint64_t>>(rng, FLAGS_output_dir, rowCount);
    writeFile<alpha::VarintEncoding<float>>(rng, FLAGS_output_dir, rowCount);
    writeFile<alpha::VarintEncoding<double>>(rng, FLAGS_output_dir, rowCount);
  }
