...
DEFINE_bool(
    alpha_disable_coalesce,
    false,
    "Disable read coalescing in Alpha reader.");

DEFINE_uint64(
    alpha_coalesce_max_distance,
    1024 * 1024 * 1.25,
    "Maximum read coalescing distance in Alpha reader. And gap smaller than this value will be coalesced.");
...
size_t copyTo(const folly::IOBuf& source, void* target, size_t size) {
  ...
  for (const auto& chunk : source) {
    std::copy(chunk.begin(), chunk.end(), static_cast<char*>(target));
    offset += chunk.size();
  }
...
cloneAndCoalesce(const folly::IOBuf& src, size_t offset, size_t size) {
  ...
  ALPHA_ASSERT(cursor.totalLength() >= offset, "Offset out of range");
  cursor.skip(offset);
  ALPHA_ASSERT(cursor.totalLength() >= size, "Size out of range");
  folly::IOBuf result;
...
void TabletWriter::writeOptionalSection(
  ...
  ALPHA_CHECK(!name.empty(), "Optional section name cannot be empty.");
  ALPHA_CHECK(
...
void TabletWriter::tryWriteStripeGroup(bool force) {
  ...
  ALPHA_ASSERT(stripeCount == streamSizes_.size(), "Stripe count mismatch.");
  if (stripeCount == 0) {
    return;
  }
...
void Tablet::StripeGroup::reset(
  ...
  ALPHA_ASSERT(stripeCount > 0, "Unexpected stripe count");
...
Postscript Postscript::parse(std::string_view data) {
  ALPHA_CHECK(data.size() >= kPostscriptSize, "Invalid postscript length");
  ...
  ALPHA_CHECK(
      magicNumber == kMagicNumber, "Magic number mismatch. Not an alpha file!");
...
void Tablet::initStripes() {
  ...
  if (stripes_) {
    ...
    ALPHA_CHECK(stripeCount_ > 0, "Unexpected stripe count");
    ALPHA_CHECK(
...
std::vector<std::unique_ptr<StreamLoader>> Tablet::load(
  ...
  ALPHA_CHECK(stripe < stripeCount_, "Stripe is out of range.");
