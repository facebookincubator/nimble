...
std::shared_ptr<TypeBuilder> convertToAlphaType(
  ...
  switch (type.kind()) {
    ...
    case velox::TypeKind::ARRAY: {
      ...
      auto alphaType = builder.createArrayTypeBuilder();
      alphaType->setChildren(
          convertToAlphaType(builder, *arrayType.elementType()));
      return alphaType;
    }
    case velox::TypeKind::MAP: {
      auto& mapType = type.asMap();
      auto alphaType = builder.createMapTypeBuilder();
      auto key = convertToAlphaType(builder, *mapType.keyType());
      auto value = convertToAlphaType(builder, *mapType.valueType());
      alphaType->setChildren(key, value);
      return alphaType;
    }
    case velox::TypeKind::ROW: {
      auto& rowType = type.asRow();
      auto alphaType = builder.createRowTypeBuilder(rowType.size());
      for (size_t i = 0; i < rowType.size(); ++i) {
        alphaType->addChild(
            rowType.nameOf(i),
            convertToAlphaType(builder, *rowType.childAt(i)));
      }
      return alphaType;
    }
    default:
