...
void testChunks(
    ...
    std::vector<std::tuple<velox::VectorPtr, alpha::FlushDecision>> vectors,
    std::function<void(const alpha::Tablet&)> verifier,
  ...
  auto flushDecision = alpha::FlushDecision::None;
  alpha::VeloxWriter writer(
      rootPool,
      type,
      std::move(writeFile),
...
TEST_F(VeloxWriterTests, ChunkedStreamsRowAllNullsNoChunks) {
  ...
  testChunks(
      *rootPool_,
      ...
      {{vector, alpha::FlushDecision::None},
       {vector, alpha::FlushDecision::None}},
      [&](const auto& tablet) {
        ...
        alpha::InMemoryChunkedStream chunked{
...
TEST_F(VeloxWriterTests, ChunkedStreamsRowAllNullsWithChunksMinSizeBig) {
  ...
  testChunks(
      *rootPool_,
      ...
      {{vector, alpha::FlushDecision::Chunk},
       {vector, alpha::FlushDecision::Chunk}},
      [&](const auto& tablet) {
        ASSERT_EQ(1, tablet.stripeCount());
...
TEST_F(VeloxWriterTests, ChunkedStreamsRowSomeNullsNoChunks) {
  ...
  testChunks(
      *rootPool_,
      /* minStreamChunkRawSize */ 0,
      {{nullsVector, alpha::FlushDecision::None},
       {nonNullsVector, alpha::FlushDecision::None}},
      [&](const auto& tablet) {
        ASSERT_EQ(1, tablet.stripeCount());
...
TEST_F(VeloxWriterTests, ChunkedStreamsRowSomeNullsWithChunksMinSizeBig) {
  ...
  testChunks(
      *rootPool_,
      /* minStreamChunkRawSize */ 1024,
      {{nullsVector, alpha::FlushDecision::Chunk},
       {nonNullsVector, alpha::FlushDecision::Chunk}},
      [&](const auto& tablet) {
        ASSERT_EQ(1, tablet.stripeCount());
